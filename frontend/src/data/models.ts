...
export async function importModelFromURL(url: string, scope: ModelScope, namespace?: string): Promise<ModelItem> {
  ...
  return addModel({
    ...
    uri,
    scheme: uri.startsWith('hf://') || uri.startsWith('hf-mirror://') ? 'hf' : 'modelscope',
    tags: ['imported'],
    modelType: 'llm',
    sizeGB: 0,
    readme: `# ${name}\n\nImported from ${url}.`,
